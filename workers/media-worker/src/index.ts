...
import { convertMedia, getOutputExtension } from "./converters/media-converter.js";
...
async function processConvertJob(job: Job<ConvertJobData>): Promise<void> {
  ...
  logger.info({ jobId, converterId, fileCount: inputFiles.length }, "Starting media conversion");
  ...
  for (let i = 0; i < inputFiles.length; i++) {
    ...
    try {
      const outputPath = await convertMedia(file.path, outputDir, outputFormat, {
...
const worker = new Worker<ConvertJobData>("media-convert", processConvertJob, {
  connection,
  concurrency,
  limiter: {
    max: 2,
